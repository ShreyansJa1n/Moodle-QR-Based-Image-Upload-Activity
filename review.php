...
// echo "<h2 style='padding-left: 420px'>{$attemptid}</h2>";

include('url.php');

$idata =$DB->get_records_sql("SELECT mdl_images.image,mdl_images.slot  FROM mdl_images JOIN mdl_randomnumber ON mdl_images.ran_num=mdl_randomnumber.ran_num WHERE mdl_randomnumber.attemptid=$attemptid");
...
foreach($idata as $img){
    echo "<script>
        $(document).ready( function(){
            $('#{$img->slot}').append(' <a ". 'target="_blank" rel="noopener noreferrer" ' ."href=$url/{$img->image}> Image {$img->image} </a> <br> ');
        } );
    </script>";
}


// echo "<a href=http://localhost:8888/moodle311/{$idata}>Image1</a>";
