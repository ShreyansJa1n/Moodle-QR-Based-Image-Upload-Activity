...
//DB For links
$idata =$DB->get_records_sql("SELECT mdl_images.image FROM mdl_images JOIN mdl_randomnumber ON mdl_images.ran_num=mdl_randomnumber.ran_num WHERE mdl_randomnumber.attemptid=$attemptid AND slot=$slot");
...
//Outputing links to FE
foreach ($idata as $img){
    echo "<div style='padding:50px'> <a ". 'target="_blank" rel="noopener noreferrer" ' ."href=$url/{$img->image}> Image {$img->image} </a> <br> </div>";
}
?>
<div>
